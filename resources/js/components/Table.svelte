<script>
    ...
    $: visibleLinks = buildVisibleLinks(
        serverSideData?.links || [],
        serverSideData?.current_page || 1,
    );
    ...
    function buildVisibleLinks(links, currentPage) {
        ...
        const numberedLinks = links.filter(
            (l) => l.url && !isNaN(parseInt(l.label)),
        );
    ...
    function handleFilters() {
        ...
        Object.keys(filterClientData).forEach((key) => {
            if (
                Array.isArray(filterClientData[key]) &&
                filterClientData[key].length > 0
            ) {
                params[key + "[]"] = filterClientData[key];
            }
        });
        ...
        >
            <div class="text-sm text-gray-500 flex items-center gap-2">
                ...
                <span class="font-medium text-gray-700"
                    >{serverSideData.from || 0}</span
                >
                <span>a</span>
                <span class="font-medium text-gray-700"
                    >{serverSideData.to || 0}</span
                >
                <span>de</span>
                <span class="font-medium text-gray-700"
                    >{serverSideData.total || 0}</span
                >
                <span>entradas</span>
                <select
                ...
                {#each visibleLinks as link, i}
                    {#if link.url === null}
                        <span class="px-3 py-1 text-gray-400 cursor-not-allowed"
                            >{link.label
                                .replace(/&laquo;/g, "<")
                                .replace(/&raquo;/g, ">")}</span
                        >
                    {:else if link.active}
                        <span
                            class="px-3 py-1 bg-yellow/70 font-bold border rounded-xl"
                            >{link.label}</span
                        >
                    {:else}
                        <a
